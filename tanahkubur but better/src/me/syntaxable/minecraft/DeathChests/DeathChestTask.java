	...
	public void run() {
		...
		if (! blockIsChestable(deathLocation.getBlock()) ) {
			if (blockIsChestable(deathLocation.getBlock().getRelative(BlockFace.NORTH)))
				...
				player.sendMessage(ChatColor.GREEN + "Your items are on the ground. Hurry up and retrieve them!");
				if (debug) {
					player.sendMessage(ChatColor.GREEN + "Locations examined were NOT suitable for a chest.");
		...
		if (ground_count > 0) {
			player.sendMessage(ChatColor.GREEN + "Some of your items have been safely stored in a chest at your death site.");
			player.sendMessage(ChatColor.GREEN + "Hurry up and retrieve the rest!");
		} else {
			player.sendMessage(ChatColor.GREEN + "All of your items have been safely stored in a chest at your death site.");
		}
